...
export class CharacterSystem extends TypeDataModel<CharacterDataSchema, WvActor> {
...
export default class WvActor extends Actor<"character"> {
    ...
    get race(): Race {
        return (
            this.items.find(
                (item): item is Race => item.type === TYPES.ITEM.RACE
            ) ??
            new Race(
                {
    ...
    get damageThreshold(): number {
        return this.system.equipment.damageThreshold.total;
    }

    /** Get the readied Item of this actor, if it exists. */
    get readiedItem(): WvItem | null {
        const itemId = this.system.equipment.readiedItemId;
    ...
    get armorApparel(): Apparel | null {
        const armorId = this.system.equipment.armorSlotId;
    ...
    get clothingApparel(): Apparel | null {
        const clothingId = this.system.equipment.clothingSlotId;
    ...
    get eyesApparel(): Apparel | null {
        const eyesId = this.system.equipment.eyesSlotId;
    ...
    get mouthApparel(): Apparel | null {
        const mouthId = this.system.equipment.mouthSlotId;
    ...
    get beltApparel(): Apparel | null {
        const beltId = this.system.equipment.beltSlotId;
    ...
    getWeaponSlotWeapon(slot: 1 | 2): Weapon | null {
        const itemId = this.system.equipment.weaponSlotIds[slot - 1];
    ...
    async readyItem(
        ...
        let apCostComposite = this.system.equipment.equipActionCosts.readyDirect;
        let quickSlots = this.quickSlots.value;
        if (item.type === "weapon") {
            if (this.system.equipment.weaponSlotIds.includes(id)) {
                apCostComposite =
                    this.system.equipment.equipActionCosts.readyFromSlot;
            }
            ...
            apCostComposite = this.system.equipment.equipActionCosts.readyFromSlot;
            quickSlots -= 1;
        }
    ...
    async unreadyItem() {
        ...
        const apCost = this.system.equipment.equipActionCosts.unready.total;
        if (this.actionPoints.value < apCost)
            throw new SystemRulesError(
                "Not enough action points.",
                "wv.system.messages.notEnoughAp"
    ...
    rollSpecial(name: SpecialName, options?: RollOptions): void {
        const special = this.system.specials[name];
        const components = [
    ...
    rollSkill(name: SkillName, options?: RollOptions): void {
        const skill = this.system.skills[name];
        this.rollAndCreateMessage(
    ...
    override prepareDerivedData(): void {
        ...
        this.system.secondary.applySizeCategory(
            this.system.background.size.total
        );
        this.system.vitals.applySizeCategory(
            this.system.background.size.total
        );
